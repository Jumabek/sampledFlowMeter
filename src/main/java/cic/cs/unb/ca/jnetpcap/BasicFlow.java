...
public class BasicFlow {
	...
	private long convertToDecimal(byte[] address){
		long integerAdress = 0;
		if (address.length==4){
			for (int i = 0; i<4; i++){
				int t = address[i]+128;
				//System.out.print(String.format("Adress[%d] = %d\t",i,t));
	...
	public long getIntIPHash(){
		...
		long hash = 17;
		long sourceIP = ipAsNumeric(FormatUtils.ip(src));
		long destIP = ipAsNumeric(FormatUtils.ip(dst));
...
//			System.out.println(this.getFlowId());
//			System.out.println(String.format("Source IP = %d",sourceIP));
//			System.out.println(String.format("Dest IP = %d",destIP));
...
//			System.exit(1);
    ...
    public String dumpFlowBasedFeaturesEx() {
    	...
    	dump.append(getIntIPHash()).append(separator);								//0
		...
		dump.append(flowStartTime).append(separator);									//7 in MCS
    	...
    	if(fwdPktStats.getN() > 0L) {
    		...
    		dump.append(String.format("%.2f",fwdPktStats.getMean())).append(separator);					//15
    		dump.append(String.format("%.2f",fwdPktStats.getStandardDeviation())).append(separator);		//16
    	}else {
    	...
    	if(bwdPktStats.getN() > 0L) {
    		...
    		dump.append(String.format("%.2f",bwdPktStats.getMean())).append(separator);					//19
    		dump.append(String.format("%.2f",bwdPktStats.getStandardDeviation())).append(separator);		//20
		}else{
    		dump.append(0).append(separator);
    		dump.append(0).append(separator);
    		dump.append(0).append(separator);
    	...
    	if (flowDuration<1)
		{
			dump.append(-1).append(separator);//21
			dump.append(-1).append(separator);//22
		}
    	else {
			dump.append(String.format("%.2f", ((double) (forwardBytes + backwardBytes)) / ((double) flowDuration / 1000000L ))).append(separator);//21
			dump.append(String.format("%.2f", ((double) packetCount()) / ((double) flowDuration / 1000000L))).append(separator);//22
		}
        ...
        if (flowDuration==0)
        {
    	    ...
    	    dump.append(String.format("%.2f",flowIAT.getMean())).append(separator);							//23
    	    dump.append(String.format("%.2f",flowIAT.getStandardDeviation())).append(separator);				//24
    	...
    	if(this.forward.size()>1){
        	...
        	dump.append(String.format("%.2f",forwardIAT.getMean())).append(separator);					//28
        	dump.append(String.format("%.2f",forwardIAT.getStandardDeviation())).append(separator);		//29
    	...
    	if(this.backward.size()>1){
        	...
        	dump.append(String.format("%.2f",backwardIAT.getMean())).append(separator);					//33
        	dump.append(String.format("%.2f",backwardIAT.getStandardDeviation())).append(separator);		//34
		...
		dump.append(String.format("%.2f",getfPktsPerSecond())).append(separator);							//43
		dump.append(String.format("%.2f",getbPktsPerSecond())).append(separator);							//44
		
		
		if(this.forward.size() > 0 || this.backward.size() > 0){
			dump.append(flowLengthStats.getMin()).append(separator);				//45
			dump.append(flowLengthStats.getMax()).append(separator);				//46
			dump.append(String.format("%.2f",flowLengthStats.getMean())).append(separator);				//47
			dump.append(String.format("%.2f",flowLengthStats.getStandardDeviation())).append(separator);	//48
			dump.append(String.format("%.2f",flowLengthStats.getVariance())).append(separator);			//49
		...
		dump.append(String.format("%.2f",getAvgPacketSize())).append(separator);							//59
		dump.append(String.format("%.2f",fAvgSegmentSize())).append(separator);							//60
		dump.append(String.format("%.2f",bAvgSegmentSize())).append(separator);							//61
